...
export default function TableData() {
  ...
  const handleChangePage = (event, newPage) => {
    // Check if there are no visibleRows on the current page
    if (visibleRows.length === 0) {
      // If there are no visibleRows on the current page, navigate to the previous page if available
      const nextPage = Math.max(page - 1, 0);
      setPage(nextPage);
    } else {
      // If there are visibleRows on the current page, set the page state to the new page value
      setPage(newPage);
    }
  };
  ...
  const visibleRows = useMemo(
    () => stableSort(reversedRows, getComparator(order, orderBy)).slice(page * rowsPerPage, (page + 1) * rowsPerPage),
    [reversedRows, order, orderBy, page, rowsPerPage]
  );
      ...
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        >
          ...
          <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle">
            ...
            />
            {visibleRows.length === 0 ?  <TableCell colSpan={4} align="center">
                    No Data Found
                  </TableCell> :
              <TableBody>
